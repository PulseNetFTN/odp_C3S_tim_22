...
interface HealthCardProps {
    name: string;
    status: { status: 'up' | 'down'; latency: number; lastChecked: string };
    port: number;
}
        ...
        >
            ...
            <div className="relative z-10">
                <div className="flex items-center justify-between mb-3">
                    ...
                    <div className={`w-2 h-2 rounded-full ${status?.status === 'up' ? 'bg-green-500' : 'bg-red-500'} animate-pulse`} />
                </div>
                <div className="space-y-2">
                    <div className="flex justify-between text-xs">
                        <span className="text-muted-ghost">Status</span>
                        <span className={status?.status === 'up' ? 'text-green-400' : 'text-red-400'}>
...
export default function HealthMonitor({ onRefresh }: Props) {
    ...
    const handleFailover = async () => {
        ...
        const slaveIndex = health.slave1?.status === 'up' ? 0 : health.slave2?.status === 'up' ? 1 : null;
    ...
    const nodes = [
        { name: 'Master', status: health.master, port: 3306 },
        { name: 'Slave 1', status: health.slave1, port: 3307 },
        { name: 'Slave 2', status: health.slave2, port: 3308 },
    ];
        ...
        <div>
            ...
            <div className="relative rounded-xl p-5" style={{
                ...
                <div className="flex items-center justify-between">
                    ...
                    <span className={`text-sm font-mono ${health.replicationDelay > 5 ? 'text-yellow-400' : 'text-green-400'}`}>
                        {health.replicationDelay}s
                    </span>
                </div>
                <div className="mt-2 w-full bg-surface-base rounded-full h-1">
                    <div
                        className={`h-1 rounded-full transition-all ${health.replicationDelay > 5 ? 'bg-yellow-400' : 'bg-green-400'}`}
                        style={{ width: `${Math.min(100, (health.replicationDelay / 30) * 100)}%` }}
                    />
                </div>
                <p className="text-xs text-muted-ghost mt-2">
                    {health.replicationDelay === 0
                        ? 'Replication is fully synchronized'
                        : health.replicationDelay > 10
